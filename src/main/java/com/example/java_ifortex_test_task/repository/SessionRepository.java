...
public interface SessionRepository extends JpaRepository<Session, Long> {
    ...
    @Query(value = "SELECT s.id, (s.device_type - 1) AS device_type, s.ended_at_utc, s.started_at_utc, s.user_id " +
            "FROM sessions s " +
            "WHERE s.device_type = :#{T(com.example.java_ifortex_test_task.entity.DeviceType).DESKTOP.code} " +
